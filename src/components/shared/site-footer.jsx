  ...
  const socials = [
    {
      label: "Instagram",
      icon: <InstagramIcon />,
      href: "https://www.instagram.com/arviawellness",
    },
    {
      label: "Facebook",
      icon: <FacebookIcon />,
      href: "https://www.facebook.com/profile.php?id=61582036198588",
    },
  ];
    ...
    <footer className="border-t border-[rgba(212,175,55,0.18)] bg-[#132855]">
      <div className="mx-auto grid w-full max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.1fr_repeat(4,0.7fr)] lg:px-8">
        ...
        {Object.entries(footerLinks).map(([title, links]) => (
          <div key={title}>
            ...
            <div className="mt-4 space-y-3 text-sm text-white/70">
              {title === "Company" &&
                links.map((item) => (
                  <Link key={item.href} href={item.href} className="block transition hover:text-[#D4AF37] hover:translate-x-1">
                    {item.label}
                  </Link>
                ))}
              ...
              {title === "Treatments" &&
                links.map((item) => (
                  <Link key={item.slug} href={`/services/${item.slug}`} className="block transition hover:text-[#D4AF37] hover:translate-x-1">
                    {item.label}
                  </Link>
                ))}
        ...
        <div>
          ...
          >
            <span className="text-white">Franchise Opportunities</span>
          </Link>

          <div className="mt-6 flex flex-col w-max items-start gap-4 px-4 py-2 text-sm text-white/75">

            {/* PHONE */}
            <a href="tel:+917678115756" className="flex items-center gap-2 hover:underline">
              <Phone className="h-4 w-4" />
              <span className="border-b text-sm">+91 7678115756</span>
            </a>

            {/* EMAIL */}
            <a
              href="mailto:wellnessarvia@gmail.com"
              className="flex items-center gap-2 hover:underline"
            >
              <Mail className="h-4 w-4" />
              <span className="border-b text-sm">wellnessarvia@gmail.com</span>
            </a>
